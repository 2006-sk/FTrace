...
export function GameShell() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => tickCall(), 400)
    ...
    <div className="pointer-events-none absolute inset-0 z-10">
      ...
      <header className="pointer-events-auto absolute left-0 right-0 top-0 flex items-center justify-between gap-4 border-b border-white/10 bg-[#1a1f2e]/90 px-4 py-3 backdrop-blur-md md:px-6">
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          <span className="rounded-full border border-[#e85d4c]/40 bg-[#e85d4c]/15 px-3 py-1 text-xs font-semibold text-[#ffb4a8]">
            Offer hour · LIVE
          </span>
        </div>
      </header>
      ...
      <Card className="pixel-frame pointer-events-auto absolute bottom-6 right-5 top-[4.5rem] flex w-[min(300px,calc(100%-2rem))] flex-col overflow-hidden md:right-6 md:top-24">
        <CardHeader className="border-b-2 border-[#0c1020] bg-[#0f1428]">
          <p className="pixel text-[9px] text-[#f0b429]">GAME LOG</p>
          <CardTitle className="pixel mt-1.5 text-[11px]">Recent deals</CardTitle>
          <CardDescription className="mt-1 text-[#8fa0c8]">
            Live feed of surplus events
          </CardDescription>
        </CardHeader>
      ...
      {view === "interior" && (
        <Card className="pixel-frame pointer-events-auto absolute bottom-6 left-5 top-[4.5rem] flex w-[min(320px,calc(100%-2rem))] flex-col overflow-hidden md:left-6 md:top-24">
          <CardHeader className="border-b-2 border-[#0c1020] bg-[#0f1428]">
            <p className="pixel text-[9px] text-[#f0b429]">REGULARS</p>
            <CardTitle className="pixel mt-1.5 text-[11px]">Customer list</CardTitle>
            <CardDescription className="mt-1 text-[#8fa0c8]">
              Call regulars during offer hours via Vapi
            </CardDescription>
          </CardHeader>
          <ScrollArea className="flex-1">
            <CardContent className="space-y-2 p-3">
          ...
          <div className="border-t-2 border-[#0c1020] bg-[#0f1428] p-3">
            <Button
              ...
              disabled={selectedIds.length === 0 || callState?.bulk}
            ...
            <div className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between md:p-5">
              <div className="min-w-0 flex-1">
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-white/50">{active?.window}</span>
                </div>
                <h3 className="truncate text-lg font-semibold">{active?.label}</h3>
                <p className="mt-0.5 text-sm text-white/65">{active?.deal}</p>
              </div>
                    ...
                    <p className="text-sm font-semibold">
                      ...
                      <span
                        className={`ml-2 text-xs ${
                          item.risk > 0.65 ? "text-[#e85d4c]" : "text-[#f0b429]"
                        }`}
                      >
